...
# Custom font generator for SyukuZen
#
...
# Set configuration
    ...
    SetPanose([3, 6, panoseweight_list[i], 3, 4, 2,\
               2, 2, 2, 5])
...
# --------------------------------------------------
    ...
    Print("Edit numbers")
# 0 (スラッシュ0にする)
    Select("zero.zero"); Copy() # zeroタグの0
    Select("zero");  Paste() # 0

# ０ (スラッシュ0にする)
    Select("zero.zero.full"); Copy() # zeroタグの０
...
# l (つま先を延ばす)
    ...
    Select(65552);  Paste() # Temporary glyph
    Select(0u25a0); Copy() # Black square
    Select(65552);  PasteWithOffset(68, -580) # Temporary glyph
    OverlapIntersect()
    Scale(220, 100, 264, 0)
    Copy()
    Select(0u006c) # l
...
# ｌ(つま先を延ばす)
    Select(0uff4c); Copy() # 全角ｌ
    Select(65552);  Paste() # Temporary glyph
    Select(0u25a0); Copy() # Black square
    Select(65552);  PasteWithOffset(360, -580) # Temporary glyph
    OverlapIntersect()
    Scale(220, 100, 556, 0)
    Copy()
    Select(0uff4c) # 全角ｌ
