...
# Custom font generator for MaruCue Pop
#
...
font_familyname="MaruCue"
font_familyname_suffix="Pop"
...
copyright="${copyright}\" + \"[knsw_HachiMaruPop]\nCopyright (c) 2019 by Nontynet. All rights reserved.\n\n"
...
name_designer="Nontynet"
name_designer_url="https://nonty.net/"
name_license="SIL Open Font License, Version 1.1"
name_license_url="https://scripts.sil.org/OFL"

em_ascent="800" # em値用 ※ win_ascent - (設定したい typo_linegap) / 2 が適正っぽい
em_descent="200" # win_descent - (設定したい typo_linegap) / 2 が適正っぽい
...
typo_descent="${em_descent}" # 縦書きに対応させない場合、linegap = 0で typo、win、hhea 全てを同じにするのが無難
 # typo_linegap="200" # 本来設定したい値 (win_ascent + win_descent = typo_ascent + typo_descent + typo_linegap)
typo_linegap="200" # 数値が大きすぎると Excel (Windows版) で文字コード 80h 以上 (おそらく) の文字がずれる (Mac版については不明)
win_ascent="1000"
win_descent="200"
...
# Set filenames
origin_regular="HachiMaruPop-Regular.ttf"
origin_bold=""
...
# Set configuration
    ...
    SetPanose([3, 5, panoseweight_list[i], 3, 4, 2,\
               2, 2, 2, 6])
...
# 0 (ドット0にする)
    ...
    Select(0u0030) # 0
    PasteWithOffset(235, 170)

# ０ (ドット0にする)
    Select(0u00b7); Copy() # Middle dot
    Select(0uff10) # 全角０
    PasteWithOffset(334, 170)
...
# , (しっぽを伸ばす)
    ...
    Move(-150, -671)
    Select(0u002c); Copy() # ,
    Select(0uffee); PasteInto() # Temporary glyph
    OverlapIntersect()
    Scale(110, 200, 190, -53)

    Select(0u25a0); Copy() # Black square
    Select(0u002c) # ,
    PasteWithOffset(-150, 37)
    OverlapIntersect()

    Select(0uffee) # Temporary glyph
    Copy()
    Select(0u002c) # ,
    PasteInto()
    RemoveOverlap()
    ChangeWeight(16); CorrectDirection()
    SetWidth(500)
...
# ， (しっぽを伸ばす)
    Select(0uff0c) # 全角，
    Clear()
    PasteWithOffset(350, 56)
    SetWidth(1000)

    Select(0uffee); Clear() # Temporary glyph

# ; (しっぽを伸ばす)
    Select(0u25a0); Copy() # Black square
    Select(0u003b) # ;
    PasteWithOffset(-150, 350)
    OverlapIntersect()
    ChangeWeight(16); CorrectDirection()
...
# ．： (拡大)
    ...
    Print("Edit Kanji")
# 㪣 (移動)
    Select(0u6572); Copy() # 敲
    Select(0u3aa3); Paste() # 㪣

# 敲 (グリフ修正)
    Select(0u25a0); Copy() # Black square
    Select(0uffee); Paste() # Temporary glyph
    Move(-348, 560)
    PasteWithOffset(-373, 0)
    PasteWithOffset(-350, -380)
    RemoveOverlap()
    Copy()
    Select(0u6572); PasteInto() # 敲
    OverlapIntersect()
    ...
    Rotate(5)
    Move(349, -172)
    PasteWithOffset(420, 530)
    PasteWithOffset(270, -693)
    RemoveOverlap()
    Select(0u654d); Copy() # 敍
    Select(0uffee); PasteInto() # Temporary glyph
    OverlapIntersect()
    Copy()

    Select(0u6572); PasteWithOffset(20, 0) # 敲
    RemoveOverlap()
    SetWidth(1000)
    Select(0uffee); Clear() # Temporary glyph
...
# 寂 (グリフ修正)
    ...
    Select(0uffee); Paste() # Temporary glyph
    Move(50, 640)
    PasteWithOffset(730, 500)
    PasteWithOffset(-373, 50)
    PasteWithOffset(-350, -399)
    RemoveOverlap()
    Copy()
    Select(0u5bc2); PasteInto() # 寂
    OverlapIntersect()
    ...
    Rotate(5)
    Move(395, -130)
    PasteWithOffset(270, -695)
    RemoveOverlap()
    Select(0u83fd); Copy() # 菽
    Select(0uffee); PasteInto() # Temporary glyph
    OverlapIntersect()
    Scale(95, 100)
    Copy()

    Select(0u5bc2); PasteWithOffset(-20, -10) # 寂
    SetWidth(1000)
    Select(0uffee); Clear() # Temporary glyph
...
# name (FontForge で著作権が更新されないため ttx で更新)
cat > ${tmpdir}/copyright.txt << _EOT_
Copyright (c) 2026 omonomo
...
do
    ...
    if [ "$(grep -m 1 "Bold Oblique" "${P%%.ttf}.ttx")" ]; then # HachiMaruPop は OS/2 Ver.3 のため7、8、9ビットが使えない
        ...
        sed -i.bak -e 's,fsSelection value="........ ........",fsSelection value="00000000 00100001",' "${P%%.ttf}.ttx"
        ...
        sed -i.bak -e 's,fsSelection value="........ ........",fsSelection value="00000000 00000001",' "${P%%.ttf}.ttx"
        ...
        sed -i.bak -e 's,fsSelection value="........ ........",fsSelection value="00000000 00100000",' "${P%%.ttf}.ttx"
        ...
        sed -i.bak -e 's,fsSelection value="........ ........",fsSelection value="00000000 01000000",' "${P%%.ttf}.ttx"
    fi
    ...
    # name (FontForgeで著作権が更新されないためここで更新)
    sed -i.bak -e "/namerecord nameID=\"0\"/{n;d;}" "${P%%.ttf}.ttx"
    sed -i.bak -e "/namerecord nameID=\"0\"/r ${tmpdir}/copyright.txt" "${P%%.ttf}.ttx"
