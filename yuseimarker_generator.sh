...
# Custom font generator for Yusei Marker
#
...
# l (つま先を追加する)
    ...
    Select(65552);  PasteWithOffset(130, 500) # Temporary glyph
    OverlapIntersect()
    Copy()
    Select(0u006c) # l
    PasteWithOffset(-98, -783)
    RemoveOverlap()
    Simplify()
    Move(-20, 0)
    SetWidth(304)
...
# { (波の先端をとがらせる)
    ...
    Select(65552);  Paste() # Temporary glyph
    Select(0u25a0); Copy() # Black square
    Select(65552);  PasteWithOffset(-680, 0) # Temporary glyph
    OverlapIntersect()
    Copy()
    Select(0u007b) # {
    PasteWithOffset(30, 26)
    RemoveOverlap()
    SetWidth(500)
    Select(65552);  Clear() # Temporary glyph
...
# } (波の先端をとがらせる)
    ...
    Select(65552);  Paste() # Temporary glyph
    Select(0u25a0); Copy() # Black square
    Select(65552);  PasteWithOffset(180, 0) # Temporary glyph
    OverlapIntersect()
    Copy()
    Select(0u007d) # }
    PasteWithOffset(-30, 26)
