...
# Custom font generator for TochinokiPop
#
...
name_designer="FONTDASU"
name_designer_url="http://fontdasu.com"
...
width_threshold="900" # 半角全角判断文字幅
width_zenkaku="1000" # 全角文字幅
scale_x_ratio="98" # 幅広のグリフを縮める際の調整倍率
...
# Set filenames
origin_regular="MochiyPopOne-Regular.ttf"
origin_bold=""
...
# Set configuration
    ...
    SetOS2Value("IBMFamily",          0x0800) # サンセリフ 分類無し
...
# ＂ (追加)
    Select(0u0022); Copy() # "
    Select(0uff02); Paste() # 全角＂
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＃ (追加)
    Select(0u0023); Copy() # #
    Select(0uff03); Paste() # 全角＃
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＄ (追加)
    Select(0u0024); Copy() # $
    Select(0uff04); Paste() # 全角＄
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＇ (追加)
    Select(0u0027); Copy() # '
    Select(0uff07); Paste() # 全角＇
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＊ (追加)
    Select(0u002a); Copy() # *
    Select(0uff0a); Paste() # 全角＊
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, -280)
    Scale(200)
    ChangeWeight(-60)
    CorrectDirection()
    SetWidth(1000)
...
# － (追加)
    Select(0u002d); Copy() # -
    Select(0uff0d); Paste() # 全角－
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＜ (追加)
    Select(0u003c); Copy() # <
    Select(0uff1c); Paste() # 全角＜
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＞ (追加)
    Select(0u003e); Copy() # >
    Select(0uff1e); Paste() # 全角＞
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＾ (追加)
    Select(0u005e); Copy() # ^
    Select(0uff3e); Paste() # 全角＾
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ＿ (追加)
    Select(0u005f); Copy() # _
    Select(0uff3f); Paste() # 全角＿
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ｀ (追加)
    Select(0u0060); Copy() # Grave accent
    Select(0uff40); Paste() # 全角｀
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)

# ｜ (追加)
    Select(0u007c); Copy() # |
    Select(0uff5c); Paste() # 全角｜
    Move(${width_zenkaku} / 2 - GlyphInfo("Width") / 2, 0)
    SetWidth(1000)
