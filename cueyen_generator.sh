...
# Custom font generator for CueYen Pop
#
...
width_expand="0" # 幅広のグリフを縮める際の両端調整幅 (マイナスで文字拡大、プラスで縮小)
    ...
    foreach
        if (WorthOutputting())
            ...
            if (glyph_width <= ${width_zenkaku})
                ...
                temp = 100 * ${width_zenkaku} / glyph_width
                Scale(temp, 100, 0, 0)
                if (0 < ${weight_narrow})
...
# 寂 (グリフ修正)
    ...
    Select(0uffee); Paste() # Temporary glyph
    Move(50, 640)
    PasteWithOffset(-50, 640)
    PasteWithOffset(730, 500)
    PasteWithOffset(-720, 500)
    RemoveOverlap()
    Copy()
    ...
    Select(0u5bc2); PasteWithOffset(-10, 0) # 寂
    SetWidth(1000)
    Select(0uffee); Clear() # Temporary glyph
