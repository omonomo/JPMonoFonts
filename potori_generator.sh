...
# Custom font generator for Potori
#
...
weight_narrow="0" # 幅広のグリフを縮める際のウェイト調整値 (正の数、小さいほど拡大、0で無効)
...
# ； (しっぽを伸ばす)
    ...
    Copy()
    Select(0uff1b) # 全角；
    PasteWithOffset(311, 90)
    SetWidth(1000)

# Grave ｀ (下げる)
    Select(0u0060) # Grave accent
    SelectMore(0uff40) # 全角｀
    Move(0, -132)
