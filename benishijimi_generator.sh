...
# Custom font generator for Benishijimi
#
...
name_designer="Yoshimichi Ohira"
name_designer_url="https://www.zenfont.jp"
...
# Set filenames
origin_regular="ZenKurenaido-Regular.ttf"
origin_bold=""
...
# ０ (スラッシュ0にする)
    ...
    Copy()
    Select(0uff10) # 全角０
    PasteWithOffset(288, 0)
    Select(0uffee);  Clear() # Temporary glyph

    Print("Edit alphabets")
# I (セリフを付ける)
    Select(0u2160); Copy() # Ⅰ
    Select(0u0049); # I
    OverlapIntersect()
    PasteWithOffset(-380, 0)
    SetWidth(239)
...
# Reverse solidus (グリフを置き換える)
    Select(0u002f); Copy() # Solidus
    Select(0u005c); Paste() # Reverse solidus
    VFlip()
    CorrectDirection()
    SetWidth(429)

# Grave (グリフを置き換える)
    Select(0uff40); Copy() # 全角Grave accent
    Select(0u0060) # Grave accent
    OverlapIntersect()
    PasteWithOffset(-282, -124)
    SetWidth(419)
...
# － (追加)
    ...
    Select(0uff0d); Paste() # Fullwidth hyphen-minus
...
# ～ (追加)
    Select(0u301c); Copy() # Wave dash
    Select(0uff5e); Paste() # Fullwidth tilde
